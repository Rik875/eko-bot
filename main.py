...
async def variant(ctx):
    ...
    variant_g = "Вот совет как можно сохранить природу: " + random.choice(v)
    await ctx.send(variant_g)

@bot.command()
async def helps(ctx):
    await ctx.send("Это бот про экологию! \n Списки команд: \n $bak + 'цвет' (Цвет писать без кавычек) - бот напишет для чего нужен бак определенного цвета \n $sites - бот напишет несколько сайтов про экологию \n $channels - каналы про экологию \n $variant - даст идею как сохранить природу")
